...
// Preserve the channel-only values documented by the original ThemeProvider.
// Complete CSS colors and var() references pass through unchanged.
const LEGACY_HSL_CHANNELS = /^\s*[+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:deg|grad|rad|turn)?(?:\s+|,\s*)[+-]?(?:\d+(?:\.\d*)?|\.\d+)%(?:\s+|,\s*)[+-]?(?:\d+(?:\.\d*)?|\.\d+)%(?:(?:\s*\/\s*|,\s*)[+-]?(?:\d+(?:\.\d*)?|\.\d+)%?)?\s*$/
...
function toStyle(theme?: ShowTheme): React.CSSProperties {
  ...
  for (const [key, value] of Object.entries(theme.colors ?? {}) as [ThemeColor, string][]) {
    if (!value) continue
    const channels = LEGACY_HSL_CHANNELS.test(value) ? value.trim() : undefined
    style[colorVars[key]] = channels ? `hsl(${channels})` : value
    const legacyVar = legacyColorVars[key]
    if (channels && legacyVar) style[legacyVar] = channels
  }
  return style
}
...
export function ThemeProvider({
  ...
  return (
    <ThemeScopeContext.Provider value={scope}>
      <div ref={setScope} data-show-theme-preset={preset} style={toStyle(theme)}>{children}</div>
