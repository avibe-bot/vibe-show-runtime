...
const BOM = "\ufeff"
// History-mode router generated by Runtime releases from #53 through #65.
// Only an exact digest match is eligible for the SSR-safe template migration.
const HISTORICAL_GENERATED_ROUTER_HASHES = new Set([
...
export async function ensureSessionTemplate(workspace: string, uiPackageName: string = DEFAULT_UI_PACKAGE) {
  ...
  if (freshWorkspace) {
    ...
    await migrateGeneratedRouter(routerPath)
  }
...
async function migrateGeneratedRouter(path: string): Promise<void> {
  let contents: string
  try {
  ...
  if (!HISTORICAL_GENERATED_ROUTER_HASHES.has(sha256(contents))) return
  await writeFile(path, routerTsx(), "utf8")
}
